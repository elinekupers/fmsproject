...
climsOneCoh = [-0.5 0.5].* max(w.V1c(1,:));
climsOneIncoh = [-0.5 0.5].* max(w.V1i(1,:));

% plot for subj 002
fH1 = figure(1); clf; set(fH1, 'Position', [1 1 1600 513], 'Name', 'Figure 4: V1 model predictions - No cancellation');

ch = subplot(221);
megPlotMap(abs(w.V1c(1,:)),climsOneCoh,[],bipolar,[],[],[],'isolines', 0.75*max(climsOneCoh) * [1 1]);
set(ch,'box','off','tickdir','out','ticklength',[0.010 0.010], 'FontSize',12); title('Coherent phase S1')

ch = subplot(222);
megPlotMap(abs(w.V1i(1,:)),climsOneIncoh,[],bipolar,[],[],[],'isolines',  0.75*max(climsOneIncoh) * [1 1])
...
climsAveCoh = [-0.5 0.5].* max(w.V1c_mn);
climsAveIncoh = [-0.5 0.5].* max(w.V1i_mn);

ch = subplot(223);
megPlotMap(abs(w.V1c_mn),climsAveCoh,[],bipolar,[],[],[],'isolines', 0.75*max(climsAveCoh) * [1 1])
set(ch,'box','off','tickdir','out','ticklength',[0.010 0.010], 'FontSize',12); title('Coherent phase Average (S1-S6)')

ch = subplot(224);
megPlotMap(abs(w.V1i_mn),climsAveIncoh,[],bipolar,[],[],[],'isolines',  0.75*max(climsAveIncoh) * [1 1])
set(ch,'box','off','tickdir','out','ticklength',[0.010 0.010], 'FontSize',12); title('Incoherent phase Average (S1-S6)')

if saveFigures
    hgexport(gcf,fullfile(figureDir, 'Figure4_predictionV1_oneVSaverage_NoCancellation.eps'));
